...
import { Plus, Mail, Link2, Phone, Users, Calendar, Award, Clock } from "lucide-react";
...
import { logContactInteraction } from "@/lib/actions/contacts";
...
export function ContactTimeline({ contactId, interactions }: { contactId: string; interactions: Interaction[] }) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      await logContactInteraction(contactId, { type, summary, details });
      setSummary("");
      setDetails("");
      setOpen(false);
    });
    ...
    <div className="flex flex-col gap-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setOpen(true)}>
          <Plus /> Ajouter une interaction
        </Button>
      </div>
      {interactions.length === 0 ? (
        <EmptyState icon={Clock} title="Aucune interaction enregistrée" />
      ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent size="sm">
          ...
          <DialogBody className="flex flex-col gap-3 pb-5">
            ...
            <Input placeholder="Résumé (ex: échange LinkedIn)" value={summary} onChange={(e) => setSummary(e.target.value)} />
