...
import { Plus, RefreshCw, Trash2, UploadCloud, CheckCircle2, XCircle, HelpCircle } from "lucide-react";
...
import { SOURCE_TYPE_LABELS } from "@/lib/discover/providers/registry";
...
export function JobSourcesSettings({ sources: initialSources }: { sources: JobSource[] }) {
  const [sources, setSources] = useState(initialSources);
  const [pending, startTransition] = useTransition();
  const [addOpen, setAddOpen] = useState(false);
  const [type, setType] = useState<"GREENHOUSE" | "LEVER" | "RSS" | "JSON_ENDPOINT" | "CSV_URL">("GREENHOUSE");
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <CardDescription>
              Chaque source est réelle et vérifiée à l&apos;ajout — Greenhouse et Lever suivent une entreprise à la fois (leur API
              publique n&apos;offre pas de recherche globale).
            </CardDescription>
          </div>
        ...
        <CardContent>
          {sources.length === 0 ? (
            <EmptyState title="Aucune source configurée" description="Ajoute une entreprise (Greenhouse/Lever), un flux RSS, ou un endpoint JSON pour commencer à peupler Discover." />
          ) : (
